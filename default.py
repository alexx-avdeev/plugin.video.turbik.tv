...
def Get(url, ref=None):
	...
	while inter:
		...
		if (http.find('<div class="loginblock" id="loginblock">') == -1):
			xbmc.log('something found')
			return http
		else:
			use_auth = True
			url = '/Signin/'
			xbmc.log('url = %s, inter = %s' % (url, inter))
		inter = inter - 1
	return None
...
def Watch(url, title, img):
	...
	Lang = 'ru'
	if (langs_en == langs_ru) and (langs_en == '1'):
		dialog = xbmcgui.Dialog()
		if dialog.select('Please choose language', ['Russian', 'English'], 3000) == 1:
			Lang = 'en'
	...
	def PlayURL(path):
		...
		if(response.status == 302):
			...
			h_1 = '|Referer=' + urllib.quote_plus('https://turbik.tv/media/swf/Player20.swf')
